    ...
    _parseCard: function (vCardData, vSiteUrl, vEtag, vDirPrefId) {
		...
		if (vCardDataArray.indexOf("VERSION:2.1") >= 0 || vCardDataArray.indexOf("version:2.1") >= 0 
			...
			try {
					...
					switch (this.findPropertyGroup(vCardDataArrayHeaderKey)) {
						...
						case "ADR":
							...
							if (vCardDataArrayTrailerTmp != null && vCardDataArrayTrailerTmp !== undefined && vCardDataArrayTrailerTmp != "") {
								...
								this.addTypeToField("adr", cardbookUtils.unescapeArray(vCardDataArrayTrailerArray), vCardDataArrayHeaderOptionArray);
							}
							break;
						case "TEL":
				...
				consoleService.logStringMessage("cardbook error with card : " + vCardData + "\n" + e);
			}
		} else {
			var consoleService = Components.classes["@mozilla.org/consoleservice;1"].getService(Components.interfaces.nsIConsoleService);
			consoleService.logStringMessage("cardbook error with card (Only 3.0 or 4.0 version supported) : \r\n" + vCardData);
		}
    }
};